class ContainsNameAtLevelFilter:
    ...
    def __call__(self, df):
        hierarchy_passes_filter = lambda hierarchy: (len(hierarchy) >= self.level) and (hierarchy[self.level-1] == self.name)
        ...
        df.loc[:, self.entity_column] = df.loc[:, self.entity_column].apply(lambda x: list(filter(hierarchy_passes_filter, x)))
        return df