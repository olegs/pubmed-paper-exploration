...
from bokeh.models import Button, Select, MultiChoice, AnnularWedge, ColumnDataSource, CustomJS
...
class InteractiveSunburst:
    ...
    def zoom_in(self, new_zoom_level: int, category: str):
        ...
        self.filters.append(ContainsNameAtLevelFilter(
...
def set_grouped_value_choice_values(interactive_sunburst: InteractiveSunburst):
    grouped_vaule_choice.options = flatten(
        results.df[get_hierarchy_column_name(interactive_sunburst.entity)].tolist())


set_grouped_value_choice_values(interactive_sunburst)
